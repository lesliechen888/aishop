...
import { getGoogleUserInfo } from '@/lib/socialAuth';
  ...
  useEffect(() => {
    const handleGoogleCallback = async () => {
      try {
        ...
        if (error) {
          setStatus('error');
          setMessage('Google登录被取消或失败');
          return;
        }

        if (!code) {
          setStatus('error');
          setMessage('未收到Google授权码');
          return;
        }

        setMessage('正在处理Google登录...');
        ...
        if (result.success) {
          ...
          setStatus('error');
          setMessage(result.error || 'Google登录失败');
        }

      } catch (error) {
        console.error('Google登录回调处理失败:', error);
        setStatus('error');
        setMessage('Google登录处理失败，请稍后重试');
      }
    };
